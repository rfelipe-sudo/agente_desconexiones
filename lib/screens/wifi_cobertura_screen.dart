...
enum _Paso { modelo, escaneoSN, verificando, menu, estado, conectando, contexto, escaneando, resultado }
...
class WifiCoberturaScreen extends StatefulWidget {
  const WifiCoberturaScreen({super.key, this.iniciarEnEscaner = false});

  /// Si es true, abre directamente el escáner de SN en lugar de la selección de modelo.
  final bool iniciarEnEscaner;
...
class _WifiCoberturaScreenState extends State<WifiCoberturaScreen>
  ...
  OntWifiService _ontWifi = OntWifiService();
  final WifiNeighborService _neighborService = WifiNeighborService();

  // Flujo ONT: escaneo SN -> buscar pass en segundo plano -> menú
  final TextEditingController _otController = TextEditingController();
  final TextEditingController _manualSNCtrl = TextEditingController();
  late final MobileScannerController _scanCtrl;
  late final AnimationController _lineCtrl;
  late final Animation<double> _lineAnim;
  bool _torchOn = false;
  double _zoom  = 0.0; // 0.0 = mínimo, 1.0 = máximo
  String? _snEscaneado;
  String? _passwordOnt;
  String? _errorPass;
  bool _buscandoPass = false;

  // Pasos de verificación visibles
  final List<_StepStatus> _verifySteps = List.filled(4, _StepStatus.waiting);
  Timer? _verifyTimer;
  ...
  void initState() {
    super.initState();
    _paso = widget.iniciarEnEscaner ? _Paso.escaneoSN : _Paso.modelo;
  ...
  Future<void> _cargarDatosSesion() async {
    ...
    final ot = prefs.getString('ot_activa') ?? prefs.getString('orden_trabajo') ?? '';
    if (ot.isNotEmpty && mounted) setState(() => _otController.text = ot);
  }
  ...
  Widget build(BuildContext context) {
    ...
    switch (_paso) {
      case _Paso.modelo:
        return _buildModelo();
      case _Paso.escaneoSN:
        return _buildEscaneoSN();
      case _Paso.verificando:
        return _buildVerificando();
      case _Paso.menu:
        return _buildMenu();
      case _Paso.estado:
        return _buildEstado();
  ...
  // ─── FLUJO ONT: ESCANEO SN → TODO EN SEGUNDO PLANO → MENÚ ────────────────

  Widget _buildEscaneoSN() {
    return Scaffold(
      backgroundColor: Colors.black,
      appBar: AppBar(
        title: const Text('Escanear SN — ONT Huawei'),
        ...
        actions: [
          IconButton(
            icon: Icon(
              ...
              color: _torchOn ? _kAccent : Colors.white70,
            ),
            onPressed: () {
      ...
      body: Stack(children: [
        ...
        LayoutBuilder(builder: (context, constraints) {
          ...
          // Franja estrecha: 52px de alto, márgenes laterales de 24px
          const margin  = 24.0;
          const stripH  = 52.0;
          final stripTop = (h - stripH) / 2;
          final scanWindow = Rect.fromLTWH(margin, stripTop, w - margin * 2, stripH);
          ...
          return Stack(children: [
            MobileScanner(
              ...
              onDetect: (capture) {
                if (_buscandoPass) return;
                final raw = capture.barcodes.firstOrNull?.rawValue;
                if (raw == null || raw.isEmpty) return;
                final sn = raw.contains(':')
                    ? raw.split(':').last.trim()
                    : raw.trim();
                if (sn.isEmpty) return;
                _buscarPasswordYConectar(sn.toUpperCase());
              },
            ),
            ...
            Positioned.fill(
              child: CustomPaint(
                painter: _OntScanOverlayPainter(scanWindow: scanWindow),
              ),
            ),
            ...
            // Instrucción encima de la franja
            Positioned(
              top: stripTop - 36,
              left: 0, right: 0,
              child: Text(
                'Apunta al código de barras del campo SN',
            ...
            Positioned(
              right: 16,
              top: stripTop + stripH / 2 - 56,
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                _zoomBtn(Icons.zoom_in, () async {
                  final next = (_zoom + 0.2).clamp(0.0, 1.0);
                  await _scanCtrl.setZoomScale(next);
                  setState(() => _zoom = next);
                }),
                ...
                Container(
                  ...
                  child: Text(
                    '${((_zoom * 4) + 1).toStringAsFixed(1)}×',
                    style: const TextStyle(color: Colors.white, fontSize: 11),
                  ),
                ),
                const SizedBox(height: 6),
                _zoomBtn(Icons.zoom_out, () async {
                  final next = (_zoom - 0.2).clamp(0.0, 1.0);
                  await _scanCtrl.setZoomScale(next);
                  setState(() => _zoom = next);
                }),
        ...
        Positioned(
          ...
          child: Container(
            ...
            child: Row(children: [
              Expanded(
                child: TextField(
                  controller: _manualSNCtrl,
                  style: const TextStyle(color: Colors.white, fontSize: 13),
                  textCapitalization: TextCapitalization.characters,
                  decoration: InputDecoration(
                    hintText: 'Ingresar SN manualmente',
              ...
              ElevatedButton(
                ...
                onPressed: () {
                  final v = _manualSNCtrl.text.trim().toUpperCase();
                  if (v.isNotEmpty) _buscarPasswordYConectar(v);
                },
                child: const Text('OK',
            ...
            child: Center(
              child: Padding(
                ...
                child: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  FilledButton.icon(
                    onPressed: () => setState(() => _errorPass = null),
  ...
  Widget _buildVerificando() {
    final steps = [
      ('Conectando con CDE Virtual',       'Portal de claves Huawei de Claro Chile'),
      ('Descargando tabla de contraseñas', '232.000 equipos con su clave de acceso'),
      ('Buscando SN en la tabla',          _snEscaneado ?? ''),
      ('Contraseña obtenida',              'Acceso root listo para configurar'),
    ];
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Verificando equipo'),
      ),
      body: SafeArea(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                ...
                child: Row(children: [
                  const Icon(Icons.router, color: _kAccent, size: 20),
                  const SizedBox(width: 10),
                  Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    const Text('Número de serie', style: TextStyle(color: _kDim, fontSize: 11)),
                    Text(
                      _snEscaneado ?? '',
                      style: const TextStyle(
  ...
  Widget _buildVerifyStep({
    ...
    switch (status) {
      ...
      case _StepStatus.error:
        ...
        icon = Icon(Icons.cancel, color: color, size: 22);
  ...
  Future<void> _buscarPasswordYConectar(String sn) async {
    if (_buscandoPass) return;

    // Resetear pasos e ir a pantalla de verificación
    for (int i = 0; i < _verifySteps.length; i++) {
      _verifySteps[i] = _StepStatus.waiting;
    }
    setState(() {
      _snEscaneado  = sn;
      _passwordOnt  = null;
      _errorPass    = null;
      _buscandoPass = true;
      _verifySteps[0] = _StepStatus.running;
      _paso = _Paso.verificando;
    });

    // Avanzar pasos con timers mientras corre el Edge Function
    final timings = [1800, 3200, 5000]; // ms hasta que arranca cada paso
    for (int i = 0; i < timings.length; i++) {
      Future.delayed(Duration(milliseconds: timings[i]), () {
        if (!mounted || !_buscandoPass) return;
        setState(() {
          _verifySteps[i]     = _StepStatus.done;
          _verifySteps[i + 1] = _StepStatus.running;
        });
      });
    }
    ...
    try {
      final resp = await Supabase.instance.client.functions
          .invoke('consultar-ont', body: {'sn': sn});
      _verifyTimer?.cancel();
      if (!mounted) return;
      ...
      if (data['error'] != null || password.isEmpty) {
        setState(() {
          for (int i = 0; i < _verifySteps.length; i++) {
            if (_verifySteps[i] == _StepStatus.running) {
              _verifySteps[i] = _StepStatus.error;
            }
          }
          _buscandoPass = false;
          _errorPass = data['error']?.toString() ??
              'SN no encontrado en la base de datos.';
        });
        return;
      }

      // Éxito: marcar todos como done y navegar
      setState(() {
        for (int i = 0; i < _verifySteps.length; i++) {
          _verifySteps[i] = _StepStatus.done;
        }
      ...
      if (!mounted) return;
      _ontWifi = OntWifiService(
          host: '192.168.1.1', user: 'root', password: password);
      setState(() {
        _passwordOnt  = password;
        _buscandoPass = false;
        _paso         = _Paso.menu;
      });
    } catch (e) {
      if (mounted) {
        setState(() {
          ...
          _buscandoPass = false;
          _errorPass    = 'Error de conexión: $e';
        });
      }
    }
  }

  Future<void> _guardarOntInstalada() async {
    final sn = _snEscaneado ?? '';
    if (sn.isEmpty) return;
    try {
      final prefs  = await SharedPreferences.getInstance();
      ...
      await Supabase.instance.client.from('ont_instaladas').upsert({
        'orden_trabajo':  ot.isEmpty ? null : ot,
        'sn_ont':         sn,
        'clave_acceso':   _passwordOnt,
        'rut_tecnico':    rut.isEmpty    ? null : rut,
        'nombre_tecnico': nombre.isEmpty ? null : nombre,
      }, onConflict: 'orden_trabajo,sn_ont');
    } catch (_) {}
  }
  ...
  Widget _buildMenu() {
    return Scaffold(
      backgroundColor: _kBg,
      appBar: AppBar(
        title: const Text('ONT Huawei'),
        backgroundColor: _kPanel,
        foregroundColor: Colors.white,
        elevation: 0,
        leading: IconButton(
          icon: const Icon(Icons.arrow_back),
          onPressed: () => setState(() => _paso = _Paso.escaneoSN),
        ),
      ),
      body: SafeArea(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              TextField(
                ...
                textCapitalization: TextCapitalization.characters,
                decoration: InputDecoration(
                  labelText: 'Orden de Trabajo (opcional)',
                  labelStyle: const TextStyle(color: _kDim),
                  hintText: '1-3ABCDEFG',
                  hintStyle: TextStyle(color: _kDim.withValues(alpha: 0.4)),
                  prefixIcon: const Icon(Icons.work_outline, color: _kAccent),
                  filled: true,
                  fillColor: _kPanel,
                  border: OutlineInputBorder(
                      borderRadius: BorderRadius.circular(12),
              ...
              _accionCard(
                ...
                titulo: 'Configurar redes',
                subtitulo: 'Cambiar SSID y contraseña WiFi de la ONT',
                onTap: () {
                  unawaited(_guardarOntInstalada());
                  Navigator.of(context).push(MaterialPageRoute(
                    builder: (_) =>
                        WifiCredencialesScreen(passwordOnt: _passwordOnt),
                  ));
                },
              ),
              const SizedBox(height: 16),
              _accionCard(
                ...
                subtitulo: 'Analizar señal WiFi y dispositivos conectados',
                onTap: () {
                  unawaited(_guardarOntInstalada());
                  _conectarYEscanear();
                },
  ...
  Future<String> _renderCertHtml() async {
    ...
    // OT y tipo de orden desde produccion_crea (Supabase) por RUT.
    String? ot;
    String? tipoOrden;
    if (rut.isNotEmpty) {
      try {
...
// Overlay para el scanner de ONT: oscurece fuera de [scanWindow]
class _OntScanOverlayPainter extends CustomPainter {
  final Rect scanWindow;
  const _OntScanOverlayPainter({required this.scanWindow});

  @override
  void paint(Canvas canvas, Size size) {
    final shadow = Paint()..color = const Color(0xBB000000);
    canvas.drawRect(Rect.fromLTRB(0, 0, size.width, scanWindow.top), shadow);
    canvas.drawRect(Rect.fromLTRB(0, scanWindow.bottom, size.width, size.height), shadow);
  ...
  @override
  bool shouldRepaint(_OntScanOverlayPainter old) => old.scanWindow != scanWindow;
}
